...
Theorem full_solution c :
  solvable c -> exists r, run_cube c r = solved_cube /\ length r <= full_bound.
Proof.
  intro Hs.
  assert (Hg : generated c) by (apply generated_solvable, Hs).
  assert (Hc0 : corners_done c []) by (intros ? []).
  assert (He0 : edges_done c []) by (intros ? []).
  ...
  assert (HDRB : read_corner (chain_state corner_stages c) DRB = (DRB, T0)).
  { apply last_corner;
      [apply generated_wellformed, HgA | apply generated_twist, HgA |].
    eapply corners_done_sub; [| exact HcA]; simpl; tauto. }
  assert (HcA8 : corners_done (chain_state corner_stages c) all_corner_slots)
    by (intros X HX; destruct X; first [exact HDRB | apply HcA; simpl; tauto]).
  assert (HeA0 : edges_done (chain_state corner_stages c) []) by (intros ? []).
  ...
  assert (HBL : fst (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)
                = BL).
  { apply eleventh_edge;
      [apply generated_wellformed, HgB | apply generated_cube_parity, HgB | exact HcB |].
    eapply edges_done_sub; [| exact HeB]; simpl; tauto. }
  assert (Hcov : edge_covers bl_table
                   (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)
                 = true)
    by (apply edge_covers_forced;
        [apply bl_table_covers_0 | apply bl_table_covers_1 | exact HBL]).
  destruct (finish_edge bl_table all_corner_slots
              [FL; FR; DB; DL; DF; DR; UB; UL; UF; UR] BL
              (chain_state edge_stages (chain_state corner_stages c))
              bl_table_ok Hcov HcB HeB) as [HcC HeC].
  pose proof (edge_lookup_bounded bl_table
                (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)
                17 bl_table_bounded Hcov) as Hl10.
  assert (HgC : generated (compose (element (edge_lookup bl_table
                   (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)))
                   (chain_state edge_stages (chain_state corner_stages c))))
    by (apply generated_step, HgB).

  (** The twelfth edge is forced. *)
  assert (HBR : read_edge (compose (element (edge_lookup bl_table
                  (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)))
                  (chain_state edge_stages (chain_state corner_stages c))) BR
                = (BR, F0)).
  { apply last_edge;
      [apply generated_wellformed, HgC | apply generated_flip, HgC |].
    eapply edges_done_sub; [| exact HeC]; simpl; tauto. }
  assert (HeC12 : edges_done (compose (element (edge_lookup bl_table
                    (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)))
                    (chain_state edge_stages (chain_state corner_stages c)))
                    all_edge_slots)
    by (intros Y HY; destruct Y; first [exact HBR | apply HeC; simpl; tauto]).
  assert (Hfin : compose (element (edge_lookup bl_table
                   (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)))
                   (chain_state edge_stages (chain_state corner_stages c)) = solved_cube)
    by (apply solved_everywhere; [exact HcC | exact HeC12]).

  (** The three sequences, joined in the order they compose. *)
  exists ((edge_lookup bl_table
             (read_edge (chain_state edge_stages (chain_state corner_stages c)) BL)
           ++ chain_word edge_stages (chain_state corner_stages c))
          ++ chain_word corner_stages c).
  split.
  - apply solution_of_left; [exact Hs |].
    rewrite <- compose_steps, <- compose_steps.
...
Theorem subgroup_solution c :
  solvable c -> in_subgroup c ->
  exists r, Forall (fun m => phase2_move m = true) r /\
            run_cube c r = solved_cube /\ length r <= domino_bound.
Proof.
  intros Hs Hsub.
  assert (Hg : generated c) by (apply generated_solvable, Hs).
  assert (Hc0 : corners_done c []) by (intros ? []).
  ...
  assert (HDRB : read_corner (after_corners c) DRB = (DRB, T0)).
  { apply last_corner;
      [apply generated_wellformed, HgA | apply generated_twist, HgA |].
    eapply corners_done_sub; [| exact HcA]; simpl; tauto. }
  assert (HcA8 : corners_done (after_corners c) all_corner_slots)
    by (intros X HX; destruct X; first [exact HDRB | apply HcA; simpl; tauto]).
  assert (HeA0 : edges_done (after_corners c) []) by (intros ? []).
  ...
  assert (HDB : read_edge (after_outer c) DB = (DB, F0)).
  { apply eighth_ud_edge; [apply generated_wellformed, HgB | exact HsB |].
    eapply edges_done_sub; [| exact HeB]; simpl; tauto. }
  assert (HeB8 : edges_done (after_outer c) ud_done)
  ...
  assert (HBLp : fst (read_edge (after_slice c) BL) = BL).
  { apply eleventh_edge;
      [apply generated_wellformed, HgC | apply generated_cube_parity, HgC | exact HcC |].
    eapply edges_done_sub; [| exact HeC]; simpl; tauto. }
  assert (HBL : read_edge (after_slice c) BL = (BL, F0)).
  { pose proof (subgroup_flip (after_slice c) BL HsC) as Hf.
    destruct (read_edge (after_slice c) BL) as [P f]; cbn [fst snd] in HBLp, Hf; subst; reflexivity. }
  assert (HBR : read_edge (after_slice c) BR = (BR, F0)).
  { apply last_edge;
      [apply generated_wellformed, HgC | apply generated_flip, HgC |].
    intros Y HY; destruct Y; simpl in HY;
      first [exact HBL | apply HeC; simpl; tauto
            | exfalso; clear -HY; intuition discriminate]. }
  assert (HeC12 : edges_done (after_slice c) all_edge_slots)
    by (intros Y HY; destruct Y;
        first [exact HBR | exact HBL | apply HeC; simpl; tauto]).
  assert (Hfin : (after_slice c) = solved_cube)
    by (apply solved_everywhere; [exact HcC | exact HeC12]).
