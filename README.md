...
correct. A heuristic is safe when it reads zero at every goal and never falls
by more than one across a step. Both conditions are decidable, so the program
checks the table it just built and `consistentb_admissible` turns a passed
check into safety. A wrong table can make the search fail or wander, but it
cannot make it accept a sequence that does not solve.
...
The main result is `solve_two_phase_sound`: on a physically valid cube, if
the solver returns a sequence, running that sequence reaches the solved cube.
The rest of this section lists what supports it and what limits it. All proofs
are checked by Rocq, with no added axioms, admitted proofs, or unchecked
computation casts.

### What is not proved

**That the solver always answers.** `solve_two_phase_sound` is conditional: it
says a returned sequence is correct, not that one is returned. The first phase
is capped at twelve moves and the second at eighteen, which suffice for every
valid cube, but that is an external fact about Kociemba's decomposition and is
assumed by no proof here.

**Anything about length.** No theorem bounds how long an answer is, and none
claims it is as short as possible. The search stops at 12 moves for the first
phase and 18 for the second, so 30 is the most it can return; measured answers
ran 19 to 25 moves. A tuned two-phase solver reaches 20 to 22 by trying
several first-phase sequences and keeping whichever leaves the shorter second
phase. This one takes the first sequence it finds and runs the second phase
once, so its answers are a few moves longer.

The 2×2×2 ancestor this repository grew from proved more on both counts: its
solver is total and its answers are at most 11 moves. Those proofs work by
computing the entire state graph inside Rocq, which is available for the
...
unproved breadth-first search and then checked. The build prints the
assumptions of the principal theorems; each reports `Closed under the global
context`.
...
The vocabulary is his: `m2f` for the function a move denotes, together with
`init_state`, `valid_state` and `run`, all used here as they were there. So is
